...
def save(simulation):
    ...
    with open("logs", 'r') as file:
        ...
        for line in lines:
            ...
            if (simulation.player.get_username() == temp[0] and simulation.player.get_password() == temp[1]):
                replacement += "{} {}".format(temp[0], temp[1])
                if (simulation.player.get_max_score() < simulation.player.get_score()):
                    replacement += " " + str(simulation.player.score)
                else:
                    replacement += " " + temp[2]
                ...
                if (simulation.player.action == 0):
                    replacement += '\n'
                else:
                    replacement += " " + str(int(simulation.player.good * 10 / simulation.player.action)) + '\n'
            else:
                replacement += line
...
def closing(root, simulation):
    '''
	Close the screen when exit button ("x") is triggered, and save data from player
...
def game_screen(game, player):
    ...
    motor1Canvas.create_text(125, 40, text="T1", font=("Arial", 10), tag="text")
    ...
    motor2Canvas.create_text(125, 40, text="T2", font=("Arial", 10), tag="text")
    ...
    motor3Canvas.create_text(125, 40, text="T3", font=("Arial", 10), tag="text")
    ...
    brokenPump11Button = Button(brokenButton, text="Broken p11", font=("Arial", 15), width=12, height=1, bg="black", bd=1, fg="white", command= lambda: broke_first_pump(simulation, tank1))
    ...
    scoreCanvas = Canvas(brokenButton, width=250, height=70)
    scoreCanvas.create_text(125, 35, text="Best Score : {}\nScore : {}".format(simulation.player.max_score, simulation.player.score), font=("Arial", 15), tag="text")
    scoreCanvas.grid(row=0, columnspan=3)

    simulation.player.set_scoreCanvas(scoreCanvas)
    ...
    game.protocol("WM_DELETE_WINDOW", lambda: closing(game, simulation))